...
import { ApiTags, ApiOperation, ApiResponse, ApiBody, ApiHeader, ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';
...
class TestMessageDto {
  @ApiProperty({ description: 'Sender phone number in E.164 format', example: '+2348012345678' })
  from: string;

  @ApiPropertyOptional({ description: 'Text body of the simulated message', example: 'Hello, I need a prescription refill' })
  body?: string;

  @ApiPropertyOptional({ description: 'URL of an image to simulate media attachment', example: 'https://example.com/prescription.jpg' })
  imageUrl?: string;
}

/**
 * DTO for test session/reset endpoints
 */
class TestSessionDto {
  @ApiProperty({ description: 'Phone number to look up or reset session for', example: '+2348012345678' })
  from: string;
}
...
export class WhatsAppWebhookController {
   ...
   */
  @ApiOperation({
    summary: 'Receive incoming WhatsApp message from Twilio',
    description: 'Main webhook endpoint that Twilio calls when a user sends a WhatsApp message. Validates the signature, parses the message, enforces rate limits, resolves identity and session, then routes through the flow engine. Returns an empty 200 response to Twilio.',
  })
  @ApiHeader({ name: 'x-twilio-signature', description: 'HMAC signature generated by Twilio for request validation', required: false })
  @ApiResponse({ status: 200, description: 'Webhook processed successfully (always returns 200 to prevent Twilio retries)' })
  @ApiResponse({ status: 403, description: 'Invalid Twilio signature (production only)' })
  @Post('twilio')
   ...
   */
  @ApiOperation({
    summary: 'Receive message delivery status callback from Twilio',
    description: 'Twilio calls this endpoint to report message delivery status updates (queued, sent, delivered, read, failed, undelivered). Logs failures for audit and monitoring.',
  })
  @ApiResponse({ status: 200, description: 'Status callback processed successfully' })
  @Post('twilio/status')
   ...
   */
  @ApiOperation({
    summary: 'Simulate an incoming WhatsApp message (dev/test only)',
    description: 'Processes a simulated WhatsApp message through the full flow engine without actually sending via Twilio. Returns the bot response instead of sending it. Disabled in production.',
  })
  @ApiBody({ type: TestMessageDto })
  @ApiResponse({ status: 200, description: 'Test message processed successfully with bot response returned' })
  @ApiResponse({ status: 400, description: 'Phone number (from) is required' })
  @ApiResponse({ status: 403, description: 'Test endpoint not available in production' })
  @Post('test')
   ...
   */
  @ApiOperation({
    summary: 'Get current session state for a phone number (dev/test only)',
    description: 'Returns the current WhatsApp identity and active session state for a given phone number. Useful for debugging conversation flows. Disabled in production.',
  })
  @ApiBody({ type: TestSessionDto })
  @ApiResponse({ status: 200, description: 'Session state retrieved successfully' })
  @ApiResponse({ status: 403, description: 'Test endpoint not available in production' })
  @Post('test/session')
  async getTestSession(
    @Body() body: TestSessionDto,
    @Res() res: Response,
  ): Promise<void> {
    if (this.configService.get('NODE_ENV') === 'production') {
      res.status(HttpStatus.FORBIDDEN).json({
   ...
   */
  @ApiOperation({
    summary: 'Reset WhatsApp session to IDLE state (dev/test only)',
    description: 'Resets the active WhatsApp session for the given phone number back to the IDLE state. Useful for testing conversation flows from the beginning. Disabled in production.',
  })
  @ApiBody({ type: TestSessionDto })
  @ApiResponse({ status: 200, description: 'Session reset to IDLE successfully' })
  @ApiResponse({ status: 403, description: 'Test endpoint not available in production' })
  @Post('test/reset')
